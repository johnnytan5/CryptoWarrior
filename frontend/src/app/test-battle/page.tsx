...
export default function TestBattlePage() {
  ...
  const handleStartBattle = async () => {
    ...
    try {
      ...
      if (!coinToUse) {
        throw new Error('No single coin with sufficient balance. Please consolidate your coins.');
      }
      ...
      let stakeCoin;
      if (coinToUse.balance > stakeAmount) {
        // Split coins returns an array, take the first element
        const [splitCoin] = tx.splitCoins(tx.object(coinToUse.object_id), [stakeAmount]);
        stakeCoin = splitCoin;
      } else {
